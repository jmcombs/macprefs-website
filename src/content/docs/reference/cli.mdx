...
**Exit codes:**
- **Pro+ tier:** `0` = PASS; `2` = ISSUES FOUND (semantic); `1` = FAIL
- **Free tier:** Always `0` for PASS/ISSUES outcomes; `1` = generic failure only

<Aside type="note">
Free tier users should rely on human-readable output for status. See [Exit Codes Reference](/reference/exit-codes/) for details.
</Aside>
...
**Exit codes:**
- **Pro+ tier:** `0` = no drift; `2` = drift detected (semantic); `1` = error
- **Free tier:** Always `0` when command completes; `1` = generic failure only
...
**Exit codes:**
- **Pro+ tier:** `0` = valid; `1` = invalid schema (semantic)
- **Free tier:** Always `0` for valid/invalid outcomes; `1` = generic failure only (missing file, malformed JSON)
...
| `activate --key <key>` | Activate with license key | Available |
| `deactivate` | Deactivate license | Available |
...
| Semantic exit codes (automation-ready) | ✗ | ✓ |
...
<Aside type="note">
Semantic exit codes (using `2` for drift/issues) are a **Pro+ feature**. Free tier always returns `0` for semantic outcomes. See [Exit Codes Reference](/reference/exit-codes/) for full details.
</Aside>
...
## Known Limitation: Array Type Coercion

When numeric values (integers, floats) or booleans are stored inside arrays or dictionaries, they may be read back as strings. For example, `[100, 200, 800, 600]` becomes `["100", "200", "800", "600"]`. This can cause false drift detection in `plan` even when preferences are functionally correct.

**Workaround:** Use string values inside arrays and dictionaries (e.g., `["100", "200"]` instead of `[100, 200]`). Top-level scalar values (integers, booleans, floats) are not affected.

This will be resolved in a future version by migrating to native CFPreferences APIs.
